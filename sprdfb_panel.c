...
static bool panel_check(struct panel_cfg *cfg)
{
	...
	case SPRDFB_PANEL_TYPE_SPI:
		cfg->panel->if_ctrl = &sprdfb_spi_ctrl;
		break;
...
{
	struct panel_cfg *cfg,*dummy_cfg;
	...
	printk(KERN_ERR "sprdfb: [%s]: Failed to match LCD Panel from uboot,use dummy lcd config !\n", __FUNCTION__);
...
static struct panel_spec *adapt_panel_from_readid(struct sprdfb_device *dev)
{
	...
	list_for_each_entry(cfg, panel_list, list) {
		...
		if(id == cfg->lcd_id) {
			...
			panel_ready(dev);
			if(cfg->panel->type == SPRDFB_PANEL_TYPE_SPI){
				cd_gpio = cfg->panel->info.spi->cd_gpio;
				if(cd_gpio) {
					if(gpio_request(cd_gpio, "cd_gpio"))
						printk("spi request cd_gpio fail\n");
				}
#ifdef CONFIG_FB_SWDISPC
				dev->spi_intf->panel = cfg->panel;
#endif
			}
			return cfg->panel;
		}
...
bool sprdfb_panel_get(struct sprdfb_device *dev)
{
	...
	if (panel) {
		...
		if(panel->type == SPRDFB_PANEL_TYPE_SPI){
			cd_gpio = panel->info.spi->cd_gpio;
			if(cd_gpio) {
				if(gpio_request(cd_gpio, "cd_gpio"))
					printk("spi request cd_gpio fail\n");
			}
#ifdef CONFIG_FB_SWDISPC
			dev->spi_intf->panel = panel;
#endif
		}
		return true;
	}
